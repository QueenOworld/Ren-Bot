...
namespace RenBot
{
    public class BankCommands : ApplicationCommandModule
    {
        public BankCommands() {
        ...
        private async Task Balance(InteractionContext ctx, [Option("user", "Who's balance to check (Default: User who ran the command)")] DiscordUser user = null)
        {
            ...
            decimal Stash = (unixTime - Convert.ToInt64(json["servers"][ctx.Guild.Id.ToString()]["bank"][user.Id.ToString()]["stash"].ToString())) * (Decimal.Round(Bank.GetCurrentValue(), 2) + 1);

            responseEmbed.AddField("Balance", "ℝ" + Decimal.Round(balance, 2).ToString());

            responseEmbed.AddField("In Stash", $"ℝ{Decimal.Round(Stash, 2).ToString()} ({Decimal.Round(unixTime - Convert.ToDecimal(json["servers"][ctx.Guild.Id.ToString()]["bank"][user.Id.ToString()]["stash"].ToString()), 2)}*{Decimal.Round(Bank.GetCurrentValue() + 1, 2)})");
        ...
        private async Task Draw(InteractionContext ctx)
        {
            ...
            decimal Stash = (unixTime - Convert.ToInt64(json["servers"][ctx.Guild.Id.ToString()]["bank"][ctx.User.Id.ToString()]["stash"].ToString())) * (Decimal.Round(Bank.GetCurrentValue(), 2) + 1);
            ...
            long cooldown = Bank.GetCooldown(Stash);
        ...
        {
            decimal currentValue = Decimal.Round(Bank.GetCurrentValue(), 2);
        ...
        private async Task Steal(InteractionContext ctx, [Option("user", "Who to steal from")] DiscordUser user)
        {
            ...
            if (Bank.SuccessfulSteal(VictimMoney))
            {
                decimal AmountToSteal = Bank.CalculateAmountToSteal(VictimMoney);
                ...
                {
                    Title = ctx.Guild.Members[ctx.User.Id].DisplayName,
                    Description = $"Successfully stole ℝ{Decimal.Round(AmountToSteal, 2)} from {ctx.Guild.Members[user.Id].DisplayName} ({Decimal.Round(Bank.StealChance(VictimMoney + AmountToSteal), 2)}% chance)",
                    Color = new DiscordColor(String.Format("#{0:X6}", RandomNumberGenerator.GetInt32(0, 0x1000000))),
                    Footer = new DiscordEmbedBuilder.EmbedFooter() { IconUrl = ctx.User.AvatarUrl }
                };
            ...
            else
            {
                ...
                {
                    Title = ctx.Guild.Members[ctx.User.Id].DisplayName,
                    Description = $"Your attempt at stealing was ℝ from {ctx.Guild.Members[user.Id].DisplayName} was unsuccessful! ({Decimal.Round(Bank.StealChance(VictimMoney), 2)}% chance)",
                    Color = new DiscordColor(String.Format("#{0:X6}", RandomNumberGenerator.GetInt32(0, 0x1000000))),
                    Footer = new DiscordEmbedBuilder.EmbedFooter() { IconUrl = ctx.User.AvatarUrl }
                };
